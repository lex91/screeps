...
const loopFunction = () => {
		...
		if (closestHostile) {
			...
			const targets = tower.room.find(FIND_STRUCTURES, {
				filter: (object) => (
					...
					object.hits / object.hitsMax < 0.99 && tower.energy / tower.energyCapacity > 0.8
				) && (
	...
	} else if (population['upgrader'].length < 2 || population['upgrader'].length == 2 && population['upgrader'][0].ticksToLive < 10) {
		utils.createUpgrader([WORK, WORK, WORK, WORK, MOVE, MOVE, MOVE, MOVE, CARRY, CARRY, CARRY, CARRY]);
